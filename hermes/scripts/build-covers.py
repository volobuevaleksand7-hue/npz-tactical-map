...
def meta_for(date, brief):
    ...
    if is_warehouse:
        # Бренд ловим и латиницей, и кириллицей: в strikes.json встречается оба
        # написания, а раньше проверялось только "wildberries" — кириллический
        # «вайлдберриз» молча уезжал в безликое «удар по логистическому складу».
        if any(k in tgt_l for k in ("wildberries", "вайлдберриз", "вайлдберис", "вайлдберрис")):
            brand = "Wildberries"
        elif any(k in tgt_l for k in ("ozon", "озон")):
            brand = "Ozon"
        else:
            brand = ""
        # Ливрея — то, по чему склад маркетплейса опознаётся мгновенно. Сам вордмарк
        # не генерим: это чужой товарный знак, и генератор всё равно ломает буквы —
        # точное «Город · удар по складу Wildberries» кладёт сверху caption_cover.py.
        livery = {"Wildberries": "ярко-фиолетовая (пурпурно-сиреневая) фирменная облицовка фасада и ворота дока",
                  "Ozon": "ярко-синяя фирменная облицовка фасада и ворота дока"}.get(brand, "")
        ...
        if livery:
            scene += (f". У склада хорошо видна {livery} — уцелевшая часть фасада сохраняет "
                      f"фирменный цвет, он читается на фоне огня и дыма")
        ...
        # Брендовый склад — как sea: img2img по случайному пресс-фото ливрею не сохранит
        # (в ref-режиме m["prompt"] вообще не используется), поэтому рисуем по тексту.
    ...
    if kind == "sea":
        ...
        # «Столб дыма на дальнем плане» генератор читал как рабочую трубу с дымком —
        # обложка 17.09 (Ярославль) показывала штатный завод. Удар должен быть виден.
        scene = ("В КАДРЕ ГЛАВНОЕ — горящая установка нефтезавода: ректификационные колонны "
                 "и резервуары в открытом пламени, стена огня и густой чёрный столб дыма "
                 "во весь кадр, искорёженные трубопроводы, работают пожарные расчёты. "
                 "Пожар занимает бОльшую часть кадра, город — лишь узкая полоса на краю")
    elif kind == "grid":
        scene = ("В КАДРЕ ГЛАВНОЕ — горящая электроподстанция: трансформаторы в пламени, "
                 "чёрный дым во весь кадр, оборванные провода на опорах, пожарные расчёты")
    elif kind == "queue":
        scene = "длинная очередь машин на заправке"
    else:
        scene = ("В КАДРЕ ГЛАВНОЕ — поражённый объект: горящее здание с чёрным столбом дыма "
                 "на переднем плане, обломки, пожарные расчёты, в небе следы ПВО")
    ...
    frame = ("широкий морской план" if kind == "sea" else
             ...
             "крупный план пожара во весь кадр, репортажный ракурс")
    prompt = (f"Дневной документальный новостной фотоснимок: {look(city, kind)}. {scene}. "
              f"Светлая ясная атмосфера, дневной свет/золотой час, фотожурналистика, НЕ мрачно и НЕ ночь. "
              f"Реализм, {frame} 1200x630 горизонталь. БЕЗ текста и букв.")
    ...
    # НПЗ/энергетика/прочее — референс тоже вреден: og:image статьи почти всегда стоковое
    # фото штатного завода, и img2img «сохраняет композицию референса» вместо пожара.
