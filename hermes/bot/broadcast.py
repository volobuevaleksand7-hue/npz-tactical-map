...
# Зеркала: та же картинка/текст, что в CHANNEL, шлём ещё и сюда. Через запятую;
# пустая строка ("") — зеркал нет. Падение зеркала НЕ должно ронять публикацию в CHANNEL.
...
def do_briefing(mode, dry=False, test_chat=None, force=False):
    ...
    # Зеркала — тот же пост в доп. каналы (id сохраняем отдельно от основного,
    ...
    for mirror in CHANNEL_MIRRORS:
        try:
            ...
            print("briefing %s: зеркало %s -> %s" % (mode, mirror, "ok" if ok else "ERR"))
            entry = {}
            if m_res and m_res[0]:
                entry["media_message_id"] = (m_res[0].get("result") or {}).get("message_id")
            if t_res and t_res[0]:
                entry["text_message_id"] = (t_res[0].get("result") or {}).get("message_id")
            if entry:
    ...
    for mirror, ids in mirrors.items():
        try:
            if mode == "caption" and not switch_to_split and ids.get("media_message_id"):
                ...
                print("update: зеркало %s editMessageCaption -> %s" % (
                    mirror, "ok" if (ok and ok.get("ok")) else ("ERR " + str(err))))
            elif mode == "split" and not switch_to_split and ids.get("text_message_id"):
                ok, err = edit_text(mirror, ids["text_message_id"], final_text)
                print("update: зеркало %s editMessageText -> %s" % (
                    mirror, "ok" if (ok and ok.get("ok")) else ("ERR " + str(err))))
            elif switch_to_split and ids.get("media_message_id"):
                ok, err = send(mirror, final_text)
                if ok and ok.get("ok"):
                    ids["text_message_id"] = (ok.get("result") or {}).get("message_id")
                    print("update: зеркало %s — переключение в split-mode." % mirror)
                else:
