#!/usr/bin/env python3
"""
@BPLAlert_bot — public-facing БПЛА alert bot (copy of @NpzFuel_Bot)
...
BOT_DIR = os.environ.get("NPZ_BPL_DIR", os.path.join(HOME, ".npz-bot-bpl"))
...
def ensure_sub(chat_id, name=""):
    ...
    sub = subs["subscribers"].setdefault(str(chat_id), {
        ...
        "src": "bot",
        "name": str(name or ""),
        "alerts": {"enabled": True, "regions": ["all"], "interval_min": 60, "threats": True, "attacks": True},
    })
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• 🔴 Угрозы (БПЛА): {'✅' if alerts.get('threats', True) else '⬜'}\n"
        f"• 💥 Атаки (НПЗ): {'✅' if alerts.get('attacks', True) else '⬜'}\n"
        f"• Подписан с: {sub.get('since', '?')}\n\n"
        f"/regions — регионы\n/interval — интервал\n/alerts — типы уведомлений\n/pause — пауза"
    )
...
# ─── Alerts toggles (threats/attacks) ───
def build_alerts_keyboard(chat_id):
    subs = load_subs()
    sub = subs["subscribers"].get(str(chat_id), {})
    alerts = sub.get("alerts", {})
    threats = alerts.get("threats", True)
    attacks = alerts.get("attacks", True)
    keyboard = [
        [InlineKeyboardButton(f"{'✅' if threats else '⬜'} 🔴 Угрозы (БПЛА/ракеты)", callback_data="alert:threats")],
        [InlineKeyboardButton(f"{'✅' if attacks else '⬜'} 💥 Атаки (удары по НПЗ)", callback_data="alert:attacks")],
    ]
    return InlineKeyboardMarkup(keyboard)
...
async def cmd_alerts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=build_alerts_keyboard(chat_id),
    )


async def on_alerts_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    _, chat_id = user_display(update)
    data = query.data  # "alert:threats" or "alert:attacks"
    field = data.split(":")[1]
    ...
    kb = build_alerts_keyboard(chat_id)
    await query.edit_message_reply_markup(reply_markup=kb)

    label = "Угрозы (БПЛА/ракеты)" if field == "threats" else "Атаки (удары по НПЗ)"
    state = "включены ✅" if alerts[field] else "выключены ⬜"
    await context.bot.send_message(chat_id=chat_id, text=f"🔔 {label}: {state}")
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        "🤖 @BPLAlert_bot — уведомления об угрозах БПЛА\n\n"
        "Команды:\n"
        ...
        "/alerts — типы уведомлений (угрозы/атаки)\n"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(on_alerts_callback, pattern="^alert:"))

    logger.info("Starting @BPLAlert_bot polling...")
