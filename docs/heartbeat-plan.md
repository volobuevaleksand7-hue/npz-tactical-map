...
| fuel-market | `15 0,6,12,18` (6ч) | 15ч (liveness-only) |
...
## Один файл — два писателя

`WATCH` — **список**, а не dict: файл не является уникальным ключом.
`data/fuel-state.json` пишут ДВА агента, каждый свою секцию:

- `npz-status` — статусы НПЗ и экспортных терминалов;
- `fuel-market` — рыночная часть (регионы дефицита, нац. баланс, меры).

Строка с `порог данных = None` — **liveness-only**: у агента нет собственного
файла, он пишет секцию чужого, поэтому свежесть этого файла ему приписать нельзя
(её бампает сосед) и в `stale_count` она не считается. Следим только за связью.

Зачем: 15.07 живой `fuel-market` каждые 6ч обновлял `generated_at` общего
`fuel-state.json` и **маскировал мёртвый `npz-status`** — данные выглядели
свежими на 2.3ч при агенте, молчавшем 21.5ч. Свежесть данных НЕ доказывает, что
агент жив; единственный честный признак живости — heartbeat.
