...
## [OPEN] cover-fallback-2026-08-20
status: OPEN
opened: 2026-08-20 08:15 МСК — summary-watchdog
Карточка за 2026-08-20 на месте, но обложка = заглушка og-image. Обычно самолечение чинит само (Codex работает и на VPS, и на Маке); если висит — вероятно кончились image-кредиты Codex-воркспейса.
Что сделать:
- `python3 hermes/scripts/build-covers.py --dates 2026-08-20` (Codex-first) → `python3 agents/gen-news.py` → git-sync + деплой;
- если Codex «out of credits» — пополнить воркспейс, либо разово `NPZ_COVERS_ALLOW_OPENROUTER=1` при живом OpenRouter-ключе.

## [OPEN] cover-fallback-2026-08-19
status: OPEN
