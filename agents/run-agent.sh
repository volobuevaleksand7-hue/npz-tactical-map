...
if ! bash agents/git-sync.sh "data(${LABEL}): sync $(date -u +%Y-%m-%dT%H:%MZ)" "$LABEL"; then
  echo "!! git-sync отказал (guard?) — откатываю только затронутые файлы"
  ...
  # 4 раза, хотя guard честно блокировал коммит самого агента. HEAD чинит и дерево,
  # и индекс — это свойство сохраняем для всего, что ниже откатываем.
  # 🔴 30.07: тот же откат, применённый ко ВСЕЙ data/, снёс 4 соседних сборщика,
  # чья работа к конфликту не имела отношения (простояли 16-35ч, 3 удара потерялись).
  # Откатываем узко:
  #   (а) пути, которых НЕ было "грязными" в DATA_BEFORE (снимок ДО этого прогона) —
  #       то, что грязным сделал ИМЕННО этот прогон (свой файл + last-sync.txt/
  #       heartbeats.json — их тоже штампует git-sync.sh на каждом прогоне);
  #   (б) плюс файлы с литеральными conflict-маркерами (тот же MARKER_RE, что и
  #       guard в git-sync.sh) — маркер никогда не бывает чьей-то легитимной
  #       работой, чей бы файл это ни был, поэтому его чистим безусловно; иначе
  #       чужой уже-грязный "брак", занесённый ДО этого прогона, блокировал бы
  #       guard всем соседям навсегда, а узкий откат его бы никогда не тронул.
  TOUCHED="$(comm -23 \
    <(git status --porcelain data/ 2>/dev/null | awk 'NF{print $2}' | sort -u) \
    <(printf '%s\n' "$DATA_BEFORE" | awk 'NF{print $2}' | sort -u))"
  MARKER_FILES="$(grep -lrE '^(<<<<<<<|=======|>>>>>>>)' data/ 2>/dev/null || true)"
  { printf '%s\n' "$TOUCHED"; printf '%s\n' "$MARKER_FILES"; } | sort -u | while IFS= read -r f; do
