...
# id завода → город/нп размещения. fuel-state.json хранит только region (область/
# край/республика), города там нет отдельным полем — для среза «НПЗ по городам»
# (реальный поисковый интент, 413 показов) города взяты из уже опубликованных
# карточек /npz/*.html (расположен в городе Х) там, где карточка это подтверждает,
# и из самого названия завода там, где оно им и является («Ачинский НПЗ» = Ачинск)
# — это устоявшееся отраслевое наименование, а не догадка. Марийский НПЗ сюда
# сознательно не включён: точный населённый пункт не подтверждён ни в данных,
# ни в его карточке — врать не будем, он останется только в разрезе по регионам.
CITY = {
    "kinef": "Кириши", "ryazan": "Рязань", "moscow": "Москва", "perm": "Пермь",
    "syzran": "Сызрань", "tuapse": "Туапсе", "kuibyshev": "Самара",
    "novokuibyshevsk": "Новокуйбышевск", "nnos": "Кстово", "yanos": "Ярославль",
    "volgograd": "Волгоград", "omsk": "Омск", "taneco": "Нижнекамск",
    "ufa": "Уфа", "achinsk": "Ачинск", "angarsk": "Ангарск",
    "komsomolsk": "Комсомольск-на-Амуре", "antipinsky": "Тюмень",
    "saratov": "Саратов", "orsk": "Орск", "afipsky": "Афипский",
    "salavat": "Салават", "ilsky": "Ильский", "slavyansk": "Славянск-на-Кубани",
    "novoshakhtinsk": "Новошахтинск", "krasnodar-rn": "Краснодар",
    "khabarovsk": "Хабаровск", "ukhta": "Ухта", "astrakhan-gpz": "Астрахань",
    "yaya": "Яя", "surgut-zsk": "Сургут", "ns-oil": "Новоспасское",
}
...
def render_cities(R):
    """«НПЗ по городам» — города известны не для всех заводов (см. CITY),
    завод без города в грид не попадает, но не пропадает со страницы —
    он остаётся в разрезе по регионам выше."""
    g = defaultdict(list)
    for r in R:
        city = CITY.get(r["id"])
        if city:
...
def render_working(R, meta):
    ...
    return (f'      <p class="lead-p">По состоянию на {date} в штатном режиме работают '
            ...
            f'нефтеперерабатывающих мощностей РФ. Это действующие НПЗ, по которым на сегодня '
...
def faq_texts(R, meta):
    ...
    return {
        ...
        "Какое количество НПЗ в России на 2026 год?":
            (f"На 2026 год количество нефтеперерабатывающих заводов в России, по данным на "
        ...
        "Сколько НПЗ осталось в России в рабочем состоянии?":
            (f"По состоянию на {date} из {tot} НПЗ России в рабочем состоянии (не остановлены "
             ...
             f"режиме и {len(part)} — с ограничениями по загрузке. Полностью выведены из строя "
             f"{len(down)} заводов."),
    }
