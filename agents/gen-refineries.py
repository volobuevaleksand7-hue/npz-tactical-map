...
def check(R, meta, html):
    ...
    # 1. Счётчики и дата в прозе/FAQ/JSON-LD
    for label, want in (("остановлен", down), ("с ограничениями", part), ("в штатном режиме", oper)):
        pass  # проверяем ниже по конкретным числам
    if f"{tot} " not in html:
        problems.append(f"число заводов {tot} не встречается в тексте")
    stale_dates = set(re.findall(r"по состоянию на (\d{1,2} \w+ \d{4})", html)) - {date}
    if stale_dates:
