...
def plural(n, one, few, many):
    """Существительное после числительного: 21 склад, 22 склада, 27 складов.

    Тот же CLDR-набор one/few/many, что в agents/sync-warehouse-counts.ru_count() —
    заведён здесь локально, чтобы не тащить importlib ради шести строк. Нужен потому,
    что число поражённых складов растёт и рано или поздно упирается в «21 складов».
    """
    n100, n10 = n % 100, n % 10
    if 11 <= n100 <= 14:
        return many
...
def build():
    ...
    # 🔴 было len(hits) — в числителе стояли поражения ОБЕИХ сетей, в знаменателе только
    # комплексы Wildberries: страница отвечала на «сколько складов ВБ пострадало» суммой
    # WB+Ozon и завышала долю сети (35/200 = 18% вместо 27/200 = 14%). Та же подмена
    # метрики, что уже ловилась на «доле заводов против доли мощностей» в gen-refineries.
    wb_hit_share = len(hit_wb) * 100.0 / net["wb"]["complexes"]
    wb_word = plural(len(hit_wb), "склад", "склада", "складов")
    hit_word = plural(len(hits), "объект", "объекта", "объектов")
    OG = og_image()
    ozon_clause = ("Ozon — 0 подтверждённых" if "ozon" not in ops
                   else "включая %d Ozon" % len(hit_oz))
    # Снипет ведёт словоформой запроса («пострадало») и сразу даёт число ПО WILDBERRIES:
    # «сколько складов вайлдберриз пострадало» — самый крупный недобор кластера
    # (1891 показ, CTR 2.1%, поз 8.9 — Вебмастер 12-26.08.2026), а суммарные 35 WB+Ozon
    # на этот вопрос отвечают неверным числом. Числа не хардкодятся: см. data/warehouses.json.
    # Конструкции «поражено N из M» выбраны намеренно — они не требуют согласования
    # существительного по числу; там, где оно нужно, стоит plural().
    DESC = ("На %s пострадало %d %s Wildberries и %d Ozon — поражено %d из %d отслеживаемых "
            "объектов, пожар подтверждён на %d, осталось %d без сообщений об ударе. "
            "Список, даты ударов и источники."
            % (rus(UPDATED), len(hit_wb), wb_word, len(hit_oz), len(hits), len(wh),
               len(burned), len(wh) - len(hits)))
    ...
    faq = [
        ...
        ("Сколько складов Wildberries осталось?",
         "Ударами напрямую поражено %d %s Wildberries из более чем %d складских комплексов сети — это около %.0f%%. Остальные в открытых данных как поражённые не значатся, но это не подтверждение того, что они работают в штатном режиме: это лишь отсутствие сообщений об ударах по ним на %s."
         % (len(hit_wb), wb_word, net["wb"]["complexes"], wb_hit_share, rus(UPDATED))),
        ...
        ("Сколько складов Wildberries пострадало?",
         "На %s в России пострадало %d %s Wildberries — это около %.0f%% от более чем %d складских комплексов сети. Ещё %d пострадавших объектов приходится на Ozon, вместе по двум сетям поражено %d из %d отслеживаемых проектом объектов. Полный список пострадавших объектов с регионом, датой удара и источником — в таблице на этой странице."
         % (rus(UPDATED), len(hit_wb), wb_word, wb_hit_share, net["wb"]["complexes"],
            len(hit_oz), len(hits), len(wh))),
        ("Какие склады Wildberries пострадали — есть ли список?",
         "Да. Таблица «Какие склады поражены» на этой странице перечисляет все %d пострадавших объекта: название, регион, дата удара и ссылка на источник по каждому (%d Wildberries и %d Ozon). Хронология эпизодов по датам — на отдельной странице «Атаки на склады Wildberries: хроника» (/ataki-na-sklady-wildberries-hronika), перечень объектов без сообщений об ударе — на странице «Какие склады Wildberries и Ozon остались» (%s)."
         % (len(hits), len(hit_wb), len(hit_oz), SURVIVORS_URL)),
        ("Сколько складов Wildberries сгорело?",
         "На %s пожар подтверждён на %d из %d поражённых складских объектов Wildberries и Ozon — это около %.0f%%. По остальным %d источники сообщают об ударе, но не подтверждают возгорание, поэтому число пострадавших складов и число сгоревших не совпадают."
         % (rus(UPDATED), len(burned), len(hits), len(burned) * 100.0 / len(hits), len(hits) - len(burned))),
...
<head>
  ...
  <meta name="keywords" content="сколько складов у wildberries, сколько складов вб, сколько складов у ozon, склады wildberries в россии количество, сколько складов сгорело, сколько складов вб сгорело, сколько осталось складов вб, сколько складов вб осталось, сколько складов вб пострадало, сколько складов вайлдберриз пострадало, сколько складов вайлдберриз пострадало в россии, сколько складов у вайлдберриз пострадало, сколько складов вб взорвали, сколько складов вб уничтожено, сколько складов вайлдберриз уничтожено, сколько складов вайлдберриз сгорело, сколько складов озон пострадало, сколько складов пострадало у вб, какие склады вб остались, вайлдберриз, валберис, площадь складов wildberries, логистика маркетплейсов россия, карта складов маркетплейсов">
  ...
  <main class="news-main">
    <div class="landing-wrap">
      ...
      <div class="landing-hero">
        ...
        <p class="hero-sub">На {rus(UPDATED)} у Wildberries и Ozon в России осталось <strong>{len(wh) - len(hits)} складов</strong> без сообщений об ударе — из {len(wh)} объектов, которые отслеживает проект ({net['wb']['complexes']}+ складских комплексов Wildberries и {net['ozon']['fulfillment']} фулфилмент-центр Ozon по данным компаний). Пострадало {len(hits)}: {len(hit_wb)} {wb_word} Wildberries и {len(hit_oz)} — Ozon; пожар подтверждён на {len(burned)}. Два маркетплейса — Wildberries (в обиходе «Вайлдберриз», «ВБ», встречается написание «Валберис») и Ozon — держат в России около {mln(net['wb']['area_m2'] + net['ozon']['area_m2'])} млн м² складов; в июле 2026 года часть этой сети выбыла после ударов беспилотников. Ниже — карта, полный список и что из этого проверяемо по открытым источникам.</p>
        ...
        <div class="status-grid">
          ...
          <div class="status-card"><div class="val">{len(hits)}</div><div class="lbl">складов пострадало от ударов</div></div>
      ...
      <p class="lead-p">Ударами беспилотников пострадало <strong>{len(hits)} {hit_word}</strong> из них: {len(hit_wb)} у Wildberries и {len(hit_oz)} у Ozon — пожар подтверждён на {len(burned)}. {"Подтверждённых ударов по складам Ozon в открытых источниках на " + rus(UPDATED) + " нет." if "ozon" not in ops else ""} Потери площадей оцениваются примерно в <strong>{LOST_M2 // 1000} тыс. м²</strong> — около <strong>{share:.0f}%</strong> складских мощностей Wildberries.</p>
      ...
      <h2 class="section-h"><span class="ico">💥</span> Сколько складов Wildberries пострадало</h2>
      <p class="lead-p">Прямой ответ на {rus(UPDATED)}: в России пострадало <strong>{len(hit_wb)} {wb_word} Wildberries</strong> — около <strong>{wb_hit_share:.0f}%</strong> сети из более чем {net['wb']['complexes']} складских комплексов. Ещё {len(hit_oz)} пострадавших объектов приходится на Ozon; вместе по двум сетям поражено {len(hits)} из {len(wh)} объектов, которые отслеживает проект.</p>
      <p class="lead-p">Что здесь значит «пострадало»: по объекту есть сообщение об ударе в открытых источниках. Степень повреждений раскрыта не по всем, поэтому число пострадавших складов и число сгоревших не совпадают — пожар подтверждён на {len(burned)}, по остальным {len(hits) - len(burned)} сообщается об ударе без подтверждения возгорания. Перечень пострадавших объектов с регионом, датой и источником — в таблице ниже; хронология эпизодов по датам — на странице <a href="/ataki-na-sklady-wildberries-hronika">«Атаки на склады Wildberries: хроника»</a>.</p>
...
{RANK_BLOCK}
      ...
      <p class="lead-p">Ударами напрямую поражено <strong>{len(hit_wb)} {wb_word}</strong> Wildberries (Вайлдберриз, ВБ) из более чем <strong>{net['wb']['complexes']} складских комплексов</strong> сети — это около <strong>{wb_hit_share:.0f}%</strong>. Остальные в открытых данных как поражённые не значатся — это не подтверждение того, что они работают в штатном режиме, а лишь отсутствие сообщений об ударах по ним.</p>
      <p class="lead-p">В вопросах читателей одно и то же событие называют по-разному: сколько складов ВБ <strong>пострадало</strong>, сколько <strong>сгорело</strong>, сколько <strong>взорвали</strong> или <strong>уничтожено</strong>. Проверяемая величина при этом одна — <strong>{len(hits)} {hit_word}</strong> с сообщениями об ударе, из них {len(hit_wb)} у Wildberries; пожар подтверждён на {len(burned)}. Данных о полном уничтожении (сносе или списании) объектов в открытых источниках нет, поэтому на этой странице используется формулировка «поражены ударами»: степень повреждений по большинству объектов официально не раскрыта.</p>
      <p class="lead-p">Итог: у Wildberries пострадало {len(hit_wb)} из {net['wb']['complexes']}+ складских комплексов, у Ozon — {len(hit_oz)}; статус оставшихся объектов проектом не проверялся и не утверждается.</p>
...
def demo():
    ...
    # 🔴 «Сколько складов ВБ пострадало» обязано отвечать числом ПО WILDBERRIES, а не
    # суммой WB+Ozon: подмена метрики уже была тут в wb_hit_share (35/200 вместо 27/200)
    # и рецидивно ловится на других страницах («доля заводов» против «доли мощностей»).
    assert "Сколько складов Wildberries пострадало" in html, "нет секции под головной запрос"
    assert "пострадало <strong>%d " % len(hit_wb) in html, (
        "прямой ответ называет не WB-число пострадавших складов")
    assert desc_m and "пострадало %d " % len(hit_wb) in desc_m.group(1), (
        "в description число пострадавших складов не по Wildberries")

    # FAQPage собирается из того же списка, что и видимый HTML, — но проверяем это
    # явно: на соседних страницах проекта разметка уже расходилась с текстом (2 из 9).
    faq_qs = re.findall(r'<div class="faq-q"[^>]*>(.*?)</div>', html)
    assert len(faq_qs) == html.count('"@type": "Question"'), (
        "число видимых вопросов не совпало с числом Question в FAQPage")
    for q in faq_qs:
        assert '"name": %s' % json.dumps(q, ensure_ascii=False) in html, (
            "вопрос FAQPage не дословно совпадает с видимым текстом: %s" % q)

    # согласование существительного после числительного — ловушка «21 складов»
    assert plural(21, "склад", "склада", "складов") == "склад"
    assert plural(22, "склад", "склада", "складов") == "склада"
    assert plural(27, "склад", "склада", "складов") == "складов"
    assert plural(11, "склад", "склада", "складов") == "складов"
