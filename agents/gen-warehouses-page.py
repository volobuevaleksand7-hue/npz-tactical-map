...
def build():
    ...
    rows = "\n".join(
        ...
        % (escape(w["name"]), escape(w["region"]), rus(w["date"]),
           escape(w.get("note", "")))
        for w in sorted(burned, key=lambda w: (w["date"], w["name"])))
    ...
    faq = [
        ...
        ("Сколько складов сгорело от ударов?",
         "По открытым данным на %s поражены %d складских объекта, все — Wildberries: %s. На всех после удара возник пожар, работа приостановлена."
         % (rus(UPDATED), len(burned), ", ".join(w["name"] for w in burned))),
        ("Пострадали ли склады Ozon?",
         "Подтверждённых в открытых источниках ударов по объектам Ozon на %s нет. Крупный пожар на складе Ozon в Истре произошёл в августе 2022 года и с ударами БПЛА не связан — на этой странице он не учитывается."
         % rus(UPDATED)),
...
<head>
  ...
  <meta property="og:image" content="https://npz-tactical-map.vercel.app/og-image.png">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="{TITLE}">
  <meta name="twitter:description" content="{DESC}">
  <meta name="twitter:image" content="https://npz-tactical-map.vercel.app/og-image.png">
  ...
  {{
    ...
    "image": ["https://npz-tactical-map.vercel.app/og-image.png"],
    ...
    <div class="landing-wrap">
      ...
      <div class="landing-hero">
        ...
        <div class="status-grid">
          ...
          <div class="status-card"><div class="val">{len(burned)}</div><div class="lbl">складов поражено ударами</div></div>
      ...
      <p class="lead-p">Ударами беспилотников в июле 2026 года поражено <strong>{len(burned)} объекта</strong> — все принадлежат Wildberries. Подтверждённых ударов по складам Ozon в открытых источниках на {rus(UPDATED)} <strong>нет</strong>. Потери площадей оцениваются примерно в <strong>{LOST_M2 // 1000} тыс. м²</strong> — около <strong>{share:.0f}%</strong> складских мощностей Wildberries.</p>

      <h2 class="section-h"><span class="ico">🔥</span> Какие склады поражены</h2>
      <div class="wh-scroll">
      <table class="wh-table">
        <thead><tr><th>Объект</th><th>Регион</th><th>Дата удара</th><th>Что известно</th></tr></thead>
        <tbody>
{rows}
        </tbody>
      </table>
      </div>
      <p class="lead-p">Хронология эпизодов с числом пострадавших и статусом работы — на странице <a href="/ataki-na-sklady-wildberries-hronika">«Атаки на склады Wildberries: хроника»</a>. Разбор первого эпизода 18 июля и версий сторон — <a href="/udar-po-skladam-wildberries">здесь</a>. Что с выплатами продавцам и покупателям — на странице <a href="/kompensacii-wildberries-posle-udara">о компенсациях</a>.</p>
      ...
      <div class="balance-box">
        <strong>Что это значит в пропорции.</strong> {LOST_M2 // 1000} тыс. м² — это заметная, но не критическая доля сети: примерно {share:.0f}% складских площадей Wildberries. Для сравнения, компания заявляла о планах ввести в 2026 году более 1,7 млн м² новых площадей — то есть годовой прирост на порядок превышает выбывшее. Ограничение оценки: компания не публикует официальных данных о потерянных площадях, цифра взята из сообщений СМИ и может быть уточнена.
      </div>
      ...
      <p class="lead-p">На слое «Склады ВБ/Озон» нанесены <strong>{len(wh)} крупных объекта</strong>: {wb_n} распределительных центров Wildberries и {oz_n} фулфилмент-центров Ozon. Поражённые ударами отмечены <strong>красным</strong> с пульсацией, работающие — фирменным цветом сети. В карточке каждого поражённого объекта — дата удара и ссылка на источник.</p>
...
{faq_html}
      ...
      <div class="link-grid">
        ...
        <a class="link-card" href="/udar-po-skladam-wildberries"><div class="lc-h">📦 Удар 18 июля: разбор</div><div class="lc-d">Что известно и почему бьют по логистике</div></a>
...
def demo():
    """Самопроверка: цифры в тексте совпадают с датасетом, Ozon не записан в поражённые."""
    with open(SRC, encoding="utf8") as f:
        doc = json.load(f)
    burned = [w for w in doc["warehouses"] if w.get("damage") == "burned"]
    assert all(w["operator"] == "wb" for w in burned), "появился поражённый Ozon — текст врёт"
    html = build()
    assert "<h1" in html and html.count("<h1") == 1
    for w in burned:
        assert w["name"] in html, w["name"]
    assert str(doc["meta"]["network"]["ozon"]["fulfillment"]) in html
    assert str(len(doc["warehouses"])) in html
