...
> ⚠️ **Где что лежит (после деанон-миграции 2026-07-07):** репо =
> `volobuevaleksand7-hue/npz-tactical-map` (push с VPS через ssh-алиас
> `github-npznew`). Старый `sergeyramas/...` — приватный бэкап, НЕ использовать.
> Полный канон расположения — в `CLAUDE.md` (блок «КАНОН РАСПОЛОЖЕНИЯ»).

Это руководство — источник правды. Прочитай целиком перед первым прогоном.

---

## 0. ⛔ ПРОТОКОЛ КООРДИНАЦИИ — читать ПЕРВЫМ (перед и после КАЖДОГО действия)

Ты не один в репо: параллельно работают другие агенты и оператор с Мака — все пушат
в `origin/main`. Чтобы не перетирать друг друга — строго по шагам.

1. **ПЕРЕД любым действием:** `cd /root/npz-tactical-map && git pull --rebase`
   (🔴 НИКОГДА `--autostash`). Прочитай `AGENT_ACTIVITY.md` (верхние записи + «Зоны
   ответственности») и `version.json`. Файл только что тронут свежей записью — не лезь,
   скоординируйся/пропусти.
1a. **Проверь инбокс инцидентов** `docs/agents/incidents.md`. Любой блок `status: OPEN` —
   задача с приоритетом сразу за указаниями оператора: выполни шаги из тела инцидента и
   поставь `status: RESOLVED`. Их пишут автосторожа (напр. `agents/summary-watchdog.py`,
   который следит, что дневная сводка `/news` и её обложка вышли к 8:00/20:00 МСК).
2. **Застолби намерение** — ОДНА строка в начало `## Log` в `AGENT_ACTIVITY.md`:
   `- <дата МСК> — intent (Hermes): <что делаю> · файлы: <…> · причина: <…>` — и сразу
   закоммить её (`git-sync.sh`), чтобы остальные видели твой lock.
3. **Работай только в своей зоне** (таблица зон в шапке `AGENT_ACTIVITY.md`): данные —
   `data/*.json`; посты/бот — `hermes/bot/`. UI-shell (`index.html`/`styles.css`/`app.js`/
   `radar.html`/`version.json`/`CHANGELOG.md`) — НЕ трогать без явного запроса оператора
   и `ALLOW_FRONTEND_RELEASE=1`.
4. **ПОСЛЕ работы — коммить СРАЗУ и только через** `bash agents/git-sync.sh "<msg>" "<hb-ключ>"`.
   🔴 ЗАПРЕЩЕНО в общем дереве: `git reset --hard`, `git checkout .`, `git stash`,
   `git pull --autostash` — молча сносят чужие незакоммиченные правки.
5. **Запиши результат** — ОДНА строка в `## Log`: `- <дата МСК> — Hermes: <что сделал> ·
   файлы · результат/проверено`.
6. **Приоритет при конфликте:** указание оператора > свежий lock в `AGENT_ACTIVITY.md` >
   `HERMES.md` > старые cron/промпты.
7. **Канон — репо** (`HERMES.md`/`AGENT_ACTIVITY.md`/`docs/`), НЕ вика-клон
   `/root/.hermes/jarvis-wiki` (он устаревает, ~5-дневное отставание). Данные о ботах:
   `docs/agents/bpla-alert-bot.md`.

---

## 0.1. TL;DR первого запуска
...
🔴 **ПЕРЕД ЛЮБОЙ правкой файла — прочитай `AGENT_ACTIVITY.md`** (зоны ответственности +
свежие блокировки), убедись что файл в твоей зоне, и запиши намерение туда до правки.
Это правило для ВСЕХ агентов/рутин, не только для UI-релизов.

🔴 **НИКОГДА не выполняй разрушительный git в общем дереве:** `git reset --hard`,
`git checkout -- .`, `git stash` (и `git pull`, который под капотом ресетит) — они
**молча сносят чужие незакоммиченные правки**. Только `agents/git-sync.sh`, который
коммитит-первым и делает `git pull --rebase` на чистом дереве. Если видишь незакоммиченные
изменения не свои — НЕ трогай, дай владельцу/автору закоммитить.
(Инцидент 2026-07-08: сторонний агент дёрнул `reset --hard origin/main` в рабочем дереве
и снёс готовые, но незакоммиченные v1.6.0-правки radar.html/прокси. Восстановлено вручную.)
...
| Кандидаты в удары (B) | `data/strike-candidates.json` | `agents/strike-candidates.py` | ежечасно :20 | **скрипт, без LLM** |
...
## 5.2. Статьи и навигация — единое дерево (ОБЯЗАТЕЛЬНО)

Полный регламент: `docs/seo-playbook.md`. Кратко, чтобы статьи не расползались:

- **Один источник правды — `data/seo-topics.jsonl`.** Каждая страница = одна строка с полем `type` (`region|object|explainer|forecast|reference`). Тип → группа меню: region→Регионы, explainer→Объяснялки, forecast→Прогноз, reference→Справочники, object(`/npz/*`)→в меню не льём, доступ через `/refineries`.
- **Меню НЕ править руками.** Оно генерится из реестра. Порядок при добавлении страницы:
  1. добавь строку в `data/seo-topics.jsonl` (в том же коммите);
  2. добавь URL в `sitemap.xml`;
  3. `python3 agents/build-nav.py` — пункт сам появится в «Аналитике» на всех страницах (красивую подпись задай в `LABELS` внутри скрипта);
  4. `python3 agents/check-ia.py` — если что-то оторвано (нет файла/sitemap/пункта меню), падает; чини до зелёного, потом пуш.
- **Дневные сводки — только в `news/ГГГГ-ММ-ДД.html`.** НЕ класть копии в корень репо (были устаревшие дубли `2026-07-07.html`/`news-07-07.html` → удалены, стоит 301 на `/news/2026-07-07`).
- Новые обложки для карточек `/analytics` рендерит Codex (`build-covers.py`), не PIL и не руками.
