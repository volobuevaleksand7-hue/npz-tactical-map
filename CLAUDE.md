...
- **Обложки постов рендерит ТОЛЬКО Codex.** Любой агент, которому нужна обложка сводки (сайт `/news` + Telegram), идёт через `python3 hermes/scripts/build-covers.py` (внутри `codex exec image_gen`, img2img по фото события). НЕ рисовать самому, НЕ звать Gemini/др. модель. PIL-фолбэк `agents/gen_cover_today.py` — только аварийно при исчерпании image-кредитов Codex: тогда пометить GENFAIL и сказать оператору пополнить кредиты, а не отгружать generic-обложку молча.
