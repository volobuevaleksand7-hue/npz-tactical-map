...
### Выкатка, переписывающая `data/*.json` целиком → сначала пауза агентам (инцидент 2026-07-29)

Релиз v1.22.0 переписал `data/fuel-availability.json` целиком (+2625 строк) за 20 минут до ротационного прохода агента, который пишет тот же файл каждые 4 часа. На hermes встал rebase, `git-sync` увидел конфликт-маркеры и (правильно) отказался пушить — **публикация замерла на 13 часов**, причём `health.json` показывал `healthy` (watchdog умер вместе с флотом).

```bash
ssh hermes-vps 'bash /root/npz-tactical-map/agents/deploy-hold.sh 10' & HOLD=$!
# ...коммит + push...
kill $HOLD   # снять паузу досрочно; иначе снимется сама через 10 мин
```

- Пауза берёт тот же `/var/lock/npz-agent.lock`, на котором агенты уже сериализуются в `run-agent.sh`: не дождавшийся агент пишет `SKIP` и выходит с 0, cron остаётся тихим. **Crontab не трогаем** — закомментированную строку легко забыть вернуть, а это freeze навсегда.
- Пауза всегда конечная (1–30 мин): бесконечная — та же авария, что чинили. `agents/deploy-hold.sh --selfcheck` сверяет, что путь лока не разошёлся с `run-agent.sh`.
- Точечные правки одного региона паузы не требуют — конфликтует именно перезапись файла целиком.
